...
MediaTek 设备 GPT 分区表读写（PGPT + SGPT）+ SP Flash Tool scatter 文件生成工具。
...
| `read-sgpt` | 从块设备提取 Secondary GPT（备份分区表项 + 备份 GPT 头） |
| `write-sgpt` | 将修改后的 sgpt 文件写回块设备（写前校验 GPT 头签名） |
...
| 输入文件 | write/write-sgpt 模式必填，要写入设备的分区表文件 |
| 块设备 | 可选，不指定则自动检测 |
...
```sh
# 提取 SGPT 到脚本目录 (默认 sgpt.bin)
sh dump_pgpt.sh read-sgpt

# 提取到指定路径
sh dump_pgpt.sh read-sgpt /sdcard/sgpt.bin

# 写入修改后的 SGPT
sh dump_pgpt.sh write-sgpt sgpt_patched.bin
```

SGPT 位于磁盘末尾，包含备份分区表项和备份 GPT 头。读取时自动从 PGPT 头的 `alt_lba` 字段定位 SGPT 区域，写入前校验文件末尾的 GPT 头签名。
...
- **sgpt** 大小 = `ceil(num_entries × entry_size / sector_size) + 1` 个扇区。包含备份分区表项（紧接在备份 GPT 头之前）和备份 GPT 头本身（位于磁盘最后一个 LBA）。
...
- 需要 **root 权限**才能读写块设备
- 写入操作会直接修改磁盘分区表，**操作前请务必备份原始 pgpt/sgpt**
