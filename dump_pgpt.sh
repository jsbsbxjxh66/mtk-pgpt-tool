...
# 从 GPT 文件解析第一个分区的起始 LBA (LE32), 用 awk 避免 shell 子串兼容问题
read_first_part_lba() {
    local file="$1" sector="$2"
    local off=$(( 2 * sector + 32 ))
    dd if="$file" bs=1 skip="$off" count=4 2>/dev/null | od -v -t x1 | awk '
